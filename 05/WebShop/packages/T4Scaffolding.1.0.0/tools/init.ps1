...
if (Test-Path $dllPath) {
	...
	if ([NuGet.PackageManager].Assembly.GetName().Version -lt 1.1) {
		...
		if ($oldVersionsInstalled) {
			Write-Warning "---"
			Write-Warning "Your solution contains older versions of T4Scaffolding. To avoid unexpected behavior, please"
			Write-Warning "uninstall the following older versions of T4Scaffolding from all projects in your solution:"
			$oldVersionsInstalled | %{ Write-Warning (" * " + $_.ToString()) }
			Write-Warning "Then restart Visual Studio."
			Write-Warning "---"
